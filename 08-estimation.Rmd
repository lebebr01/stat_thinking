...
popularity_stats <- map_dfr(1:100000, fruit_boot)
```

Once these are generated, the distribution of the mean fruit popularity can directly be shown using a density curve. Figure \@ref(fig:density-fruit-pop) shows this distribution which varies from ... to ... . This indicates a likely range of plausible values that the mean fruit popularity falls and helps to estimate the amount of uncertainty in the statistic of interest. In general, ranges that are smaller indicate less uncertainty and more accurate estimate for the statistic of interest.  

```{r density-fruit-pop, fig.cap = "Density curve of the mean fruit popularity based on 100,000 resampled data sets."}
